...
def process_execution_environment(path, logger):
    ...
    if os.path.exists(ex_env_path):
        ...
        return ex_env
    if 'dependencies' not in ex_env or not ex_env['dependencies']:
        logger.warning('No dependencies found')
        return ex_env

    python_exists = 'python' in ex_env['dependencies'] and os.path.exists(
        os.path.join(path, ex_env['dependencies']['python'])
    )
    system_exists = 'system' in ex_env['dependencies'] and os.path.exists(
    ...
    elif 'python' in ex_env['dependencies'] and not os.path.exists(
        os.path.join(path, ex_env['dependencies']['python'])
    ):
    ...
    elif 'system' in ex_env['dependencies'] and not os.path.exists(
        os.path.join(path, ex_env['dependencies']['system'])
    ):
