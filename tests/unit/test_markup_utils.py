...
def test_get_execution_environment(tmp_file):
    data = {
        "version": 1,
        "dependencies": {
            "python": [
    ...
    with open(tmp_file, 'w') as f:
        f.write(EX_ENV_YAML)
        f.flush()
        res = markup_utils.get_execution_environment(tmp_file)
        assert res == data


def test_lint_execution_environment_file(tmp_file):
    with open(tmp_file, 'w') as f:
        f.write(EX_ENV_YAML)
        f.flush()
        res = list(markup_utils.lint_execution_environment_file(tmp_file))
        assert len(res) == 1
