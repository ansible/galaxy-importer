...
def test_invalid_semver(collection_info, invalid_semver):
    ...
    with pytest.raises(ValueError, match=r"'version' to be in semantic version"):
        CollectionInfo(**collection_info)
...
def test_both_lic_and_lic_file(collection_info):
    collection_info["license_file"] = "my_very_own_license.txt"
    with pytest.raises(
        ValueError, match=r"'license' and 'license_file' keys are mutually exclusive"
    ):
        CollectionInfo(**collection_info)
...
@pytest.mark.parametrize(
    "valid_license_list",
    [["MIT"], ["Apache-2.0"], ["BSD-3-Clause"], ["CC-BY-4.0", "MIT"]],
)
...
def test_invalid_dep_type(collection_info):
    ...
    with pytest.raises(TypeError, match=r"'dependencies' must be <class 'dict'>"):
        CollectionInfo(**collection_info)
...
def test_invalid_dep_version(collection_info):
    collection_info["dependencies"] = {"joe.role1": 3}
    with pytest.raises(ValueError, match=r"Expecting depencency version to be string"):
        CollectionInfo(**collection_info)
...
def test_self_dependency(collection_info):
    ...
    collection_info["dependencies"] = {"{}.{}".format(namespace, name): "1.0.0"}
